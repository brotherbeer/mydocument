...
设2位数为〈x<sub>0</sub>, x<sub>1</sub>〉，除数为d，x<sub>0</sub>、x<sub>1</sub>、d均小于BASE，那么〈x<sub>0</sub>, x<sub>1</sub>〉在一个字长范围内，故可直接利用计算机的除法指令获取商和余数。
```C++
...
可先用t =〈x<sub>0</sub>, x<sub>1</sub>〉/〈y<sub>0</sub>, y<sub>1</sub>〉 来估商，设商的实际值为Q，t一定大于或等于Q，证明参见#9，且因为〈x<sub>0</sub>, x<sub>1</sub>〉< Y，故t必然小于BASE，即t可由一个数据单元记录。

然后再检验并修正t的值，当t * Y > X时，说明t值偏大，此时t应该适当减小。  
t * Y和X的值都超过了字长范围，故不能直接计算，但  

t * Y = t * y0 * BASE + t * y1 =〈x<sub>0</sub>, x<sub>1</sub>〉* BASE - r * BASE + t * y<sub>1</sub>

其中，r =〈x<sub>0</sub>, x<sub>1</sub>〉% Y 
...
可以再引入一次除法来加速试商过程，设d是t与实际值的差值，即〈r + d, x<sub>2</sub>〉>= (t - d) * y<sub>1</sub>，可得
...
即d *〈y<sub>0</sub>, y<sub>1</sub>〉>= t * y<sub>1</sub> -〈r, x<sub>2</sub>〉

d = t * y<sub>1</sub> -〈r, x<sub>2</sub>〉/ d + ((t * y<sub>1</sub> -〈r, x<sub>2</sub>〉) % d != 0)
...
```
需要注意的是有时r的值会超出一个数据单元的范围，即r >= BASE，如果r >= BASE，那么t * y<sub>1</sub>的值必然小于〈r, x<sub>2</sub>〉，最后〈r + d * y<sub>0</sub>, x<sub>2</sub>〉的值也可能超出字符范围，但对余数的计算并无影响，因为余数必然在字符范围内，所以在代码中，即使忽略v溢出的高位，v - u的结果总是正确的。

〈x<sub>0</sub>, x<sub>1</sub>〉<〈y<sub>0</sub>, y<sub>1</sub>〉时，本算法输出商的实际值，而当〈x<sub>0</sub>, x<sub>1</sub>〉=〈y<sub>0</sub>, y<sub>1</sub>〉时，如果仍然套用上面的算法，则t会被置为MASK即BASE - 1，这时t * y<sub>1</sub>一定小于〈r, x<sub>2</sub>〉，所以当〈x<sub>0</sub>, x<sub>1</sub>〉= Y时，会输出MASK，而实际值应为BASE，但在用到该算法的地方并不算错误，在算法D.3中会继续讨论这个问题。

对于〈x<sub>0</sub>, x<sub>1</sub>〉>〈y<sub>0</sub>, y<sub>1</sub>〉这种不予讨论，因为在这个算法被使用的过程中，这种情况根本不会发生。
...
q<sub>1</sub>, r<sub>1</sub> =〈r0, x<sub>1</sub>〉/ d,〈r0, x<sub>1</sub>〉% d
...
设
Q为X与Y进行实数除法的商，t为x<sub>0</sub>, x<sub>1</sub>, x<sub>2</sub>〉与〈y<sub>0</sub>, y<sub>1</sub>进行实数除法的商。
则
t * Y = t * (〈y<sub>0</sub>, y<sub>1</sub>〉* A + LY) =〈x<sub>0</sub>, x<sub>1</sub>, x<sub>2</sub>〉* A + t * LY

将t * Y与X进行比较，即是将t * LY与LX进行比较
1. 当t * LY <= LX时
显然，这时将t向下取整，即是整数除法的结果

2. 当t * LY > LX时
说明t大于实际的商值Q，此时t比Q大了多少呢，可以考察t-1与Q的关系。

(t - 1) * Y = (t - 1) * (〈y<sub>0</sub>, y<sub>1</sub>〉* A + LY) =〈x<sub>0</sub>, x<sub>1</sub>, x<sub>2</sub>〉* A - 〈y<sub>0</sub>, y<sub>1</sub>〉* A + (t - 1) * LY

因为y0 != 0，所以〈y<sub>0</sub>, y<sub>1</sub>〉* A是一个n位数，LY最大为n - 2位数，

如果t < BASE，(t - 1) * LY最大是n - 1位数，-〈y<sub>0</sub>, y<sub>1</sub>〉* A + (t - 1) * LY必为负数，故(t - 1) * Y 必然会小于X。这时Q∈(t-1, t)。

如果t >= BASE，t与Q的差值将难以估计，所以我们规定本算法的前提条件为〈x<sub>0</sub>, x<sub>1</sub>〉<=〈y<sub>0</sub>, y<sub>1</sub>〉，从而保证t < BASE。

结合1、2两种情况，考虑X与Y的整数除法，可以利用算法D.1求出t =〈x<sub>0</sub>, x<sub>1</sub>, x<sub>2</sub>〉/〈y<sub>0</sub>, y<sub>1</sub>〉的值，t这里为整数，则X / Y的值可能是t也可能是t - 1。
...
如果X的最高单元大于或等于Y的最高单元，则将X的最高单元更高的单元置0，m增1，从而保证了X的最高单元小于Y的最高单元，则商必为m - n位数，也满足了需要用到的算法D.3的前提条件。

任意位数之间的除法，其原理与算法D.2是相同的，可以将n位除数Y视作一个整体，利用算法D.3，每次用X的高n + 1位除以Y，得到一个商值，并将余数写回X的高n + 1位，写回余数后X的最高单元为0，即X的位数会减1，所以重复这样的除法直到X的位数与Y相同，每次求得的商是一个数据单元，将这些单元从高位到低位排列，即是原X除以Y的商，最后X的值即为余数。

有一个特殊情况，设〈x<sub>0</sub>, x<sub>1</sub>〉为X最高的两个单元，〈y<sub>0</sub>, y<sub>1</sub>〉是Y最高的两个单元，算法D.3要求〈x<sub>0</sub>, x<sub>1</sub>〉<〈y<sub>0</sub>, y<sub>1</sub>〉，经过补零等操作，第1次将X的高n+1位除以Y时，这个条件是满足的，而从第2次开始〈x<sub>0</sub>, x<sub>1</sub>〉有可能与〈y<sub>0</sub>, y<sub>1</sub>〉相等，但这并不会造成错误，因为X的高n位是上一次除法的余数，一定小于Y，而算法D.3所利用的算法D.2在这种情况下给出的结果是MASK，可以证明，当〈x<sub>0</sub>, x<sub>1</sub>〉与〈y<sub>0</sub>, y<sub>1</sub>〉相等时，本次除法商的实际值就是MASK。
