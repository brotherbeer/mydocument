 ...
 * [算法D.0 2位数除以1位数](#算法D.0-2位数除以1位数)
 * [算法D.1 3位数除以2位数](#算法D.1-3位数除以2位数)
 * [算法D.2 n位数除以1位数（n > 2）](#算法D.2-n位数除以1位数（n->-2）)
 * [算法D.3 n+1位数除以n位数（n > 3）](#算法D.3-n+1位数除以n位数（n->-3）)
 * [算法D.4 任意位数之间的除法](#算法D.4-任意位数之间的除法)
...
故d = (t * y<sub>1</sub> -〈r, x<sub>2</sub>〉) / Y + ((t * y<sub>1</sub> -〈r, x<sub>2</sub>〉) % Y != 0)
...
1. 如果t * LY <= LX  
显然，0 < X - floor(t) * Y < Y，floor(t)即是整数除法的商
...
每次用X的高n + 1位除以Y时，算法D.3的前提条件都是满足的，而且X的高n位都小于Y，每次用算法算法D.1估商的值都应该小于BASE，即可用一个数据单元记录。
